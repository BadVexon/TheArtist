...
public class AbstractPaintingCard extends AbstractArtistCard {
    ...
    private void reDescription() {
        ...
        if (greenCards > 0) {
            if (greenCards == 1) {
                rawDescription += "[#42bd41]Вы берёте " + greenCards + " [#42bd41]карту. NL ";
            } else {
                rawDescription += "[#42bd41]Вы берёте " + greenCards + " [#42bd41]карты. NL ";
            }
        }
        if (yellowEnergy > 0) {
        ...
        if (brownTemp > 0) {
            rawDescription += "[#f0749e]Даёт " + brownTemp + " [#f0749e]Временного [#f0749e]ОЗ. NL ";
        }
        if (purpCurse > 0) {
            rawDescription += "[#673ab7]Накладывает " + purpCurse + " [#673ab7]Порчи. NL ";
        }
        if (rainbowPaint > 0) {
            if (rainbowPaint == 1) {
                rawDescription += "[oobley]Нарисуйте " + rainbowPaint + " [oobley]случ. " + "[oobley]цвет. NL ";
            } else {
                rawDescription += "[oobley]Нарисуйте " + rainbowPaint + " [oobley]случ. " + "[oobley]цветов. NL ";
            }
        }
        if (yellow > 0 && red == 0 && blue == 0 && green == 0 && aqua == 0 && magenta == 0 && brown == 0 && purple == 0 && rainbow == 0)
